...
export default class VinylService {
    async identifyVinyl(photo: CameraCapturedPicture) {
        ...
        try {
            const response = await api.post('/vinyl/identifyVinyl', {
                key: key,
            })

            if (response.status == 200 && response.data.message == 'Vinyl Identified') {
                router.push(
                    `/pages/vinylResultsScreen?artist=${encodeURIComponent(response.data.artist)}&album=${encodeURIComponent(response.data.album)}`
                )
